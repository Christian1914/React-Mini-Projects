...
export default function App() {
  ...
  function handleAddWatched(newMovie) {
    setWatched((currentWatched) => {
      const isAlreadyAdded = currentWatched.some(
        (movie) => movie.imdbID === newMovie.imdbID
      );
      if (!isAlreadyAdded) {
        return [...currentWatched, newMovie];
      }
    ...
    <div className="details">
      ...
      <section>
        <div className="rating">
          {watchedMovie ? (
            <p>You rated this movie with {watchedMovie.userRating} ⭐</p>
          ) : (
            <StarRating onRatingChange={(rating) => setUserRating(rating)} />
          )}
          {/* Conditionally render the Add to List button if not already watched */}
...
function WatchedSummary({ watched }) {
  ...
  // Use Math.round to round avgRuntime to the nearest integer
